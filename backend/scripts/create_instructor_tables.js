...
async function createInstructorTables() {
  let connection;
  ...
  try {
    ...
    const [instructors] = await connection.execute('SELECT * FROM instructors');
    console.log(`✅ instructors 테이블: ${instructors.length}명의 강사`);
    instructors.forEach(instructor => {
      console.log(`   - ${instructor.name} (${instructor.department}, ${instructor.phone})`);
    });
    ...
    console.log('\n🎉 테이블 생성 완료!\n');
    console.log('이제 브라우저를 새로고침하면 강사 목록이 표시됩니다!');
